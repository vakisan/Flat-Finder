...
export class ChatDatabase{
    ...
    async sendMessage(listing,message,currentUser){
        this.createMessage(listing,message,currentUser)
        .then((resolved)=>{
            this.updateSelf(resolved).then(()=>{
                ...
                .catch((error)=>{
                    this.handleFailedMessage(resolved)
                    .then(() =>{
    ...
    async createMessage(listing,message,currentUser){
        let list = []
        return this.getMessages(listing).then(success =>{
            success.forEach(res=>{
                list.push(res.data())
            })
    ...
    handleFailedMessage(message){
        return db.collection(`Users/${message.FromUID}/ChatRooms/${message.ToUID}/Messages`)
    ...
    async updateSelf(message){
        return db.collection(`Users/${message.FromUID}/ChatRooms/${message.ToUID}/Messages`)
            .doc(message.MessageID).set(message)
    }

    async updateRecipient(message){
        console.log("Sending Message")
        return db.collection(`Users/${message.ToUID}/ChatRooms/${message.FromUID}/Messages`)
            .doc(message.MessageID).set(message)
    }
    ...
    async getMessages(listing){
        const currentUser = window._globalUserData_.UserInfo
        return db.collection(`Users/${currentUser.UserID}/ChatRooms/${listing.postedByUID}/Messages`).get()
    }
}
...
class ListingDatabase{
    ...
    async advertiseListing(listing){
        ...
        await this._getStatisticsCount('ListingCount').then((count)=>{
            const listingCount = count.data().count
            console.log(listingCount)
            db.collection('Listings').doc(`Listing${listingCount}`).set(listing)
            .then(()=>{
...
class ClientLocationDatabase{
    ...
    async getAllClientLocations(){
        ...
        const ClientLocations = await db.collection('ClientLocation').get()
        .then(list=>{
